...
// IMPORT MANAGER ROBUSTO - COMPATÍVEL COM BAILEYS COM MÚLTIPLAS ESTRATÉGIAS
class ImportManagerRobust {
  constructor(instances) {
    this.instances = instances;
    console.log('📦 ImportManagerRobust inicializado');
  }

  // Método principal de importação robusto
  async importHistory(instanceId, options = {}) {
    ...
    console.log(`[ImportManagerRobust] 📥 Iniciando importação para ${instanceId}:`, {
      importType,
      batchSize,
      lastSyncTimestamp
    });
    ...
    try {
      ...
      if (importType === 'contacts' || importType === 'both') {
        console.log(`[ImportManagerRobust] 👥 Importando contatos...`);
        ...
        result.metadata.strategiesUsed.push(`contacts:${contactsResult.source}`);
        console.log(`[ImportManagerRobust] ✅ ${result.totalContacts} contatos importados via ${contactsResult.source}`);
      }

      // Importar mensagens se solicitado
      if (importType === 'messages' || importType === 'both') {
        console.log(`[ImportManagerRobust] 💬 Importando mensagens...`);
        ...
        result.metadata.strategiesUsed.push(`messages:${messagesResult.source}`);
        console.log(`[ImportManagerRobust] ✅ ${result.totalMessages} mensagens importadas via ${messagesResult.source}`);
      }
      ...
      console.log(`[ImportManagerRobust] 🎉 Importação concluída em ${result.metadata.processingTime}ms`);
      return result;

    } catch (error) {
      console.error(`[ImportManagerRobust] ❌ Erro na importação:`, error);
  ...
  async importContactsRobust(socket, batchSize, lastSyncTimestamp) {
    ...
    for (const strategy of strategies) {
      try {
        console.log(`[ImportManagerRobust] 🔄 Tentando estratégia: ${strategy.name}`);
        const result = await strategy.method(socket, batchSize, lastSyncTimestamp);
        
        if (result.data && result.data.length > 0) {
          console.log(`[ImportManagerRobust] ✅ Sucesso com ${strategy.name}: ${result.data.length} contatos`);
          return { data: result.data, source: strategy.name };
        }
        
        console.log(`[ImportManagerRobust] ⚠️ ${strategy.name} retornou dados vazios`);
      } catch (error) {
        console.log(`[ImportManagerRobust] ❌ Falha com ${strategy.name}: ${error.message}`);
        continue;
      }
    }

    console.log(`[ImportManagerRobust] ⚠️ Todas as estratégias falharam para contatos`);
  ...
  async importMessagesRobust(socket, batchSize, lastSyncTimestamp) {
    ...
    for (const strategy of strategies) {
      try {
        ...
        if (result.data && result.data.length > 0) {
          console.log(`[ImportManagerRobust] ✅ Sucesso com ${strategy.name}: ${result.data.length} mensagens`);
          return { data: result.data, source: strategy.name };
        }
    ...
    console.log(`[ImportManagerRobust] ⚠️ Todas as estratégias falharam para mensagens`);
    return { data: [], source: 'none' };
  }
  ...
  async getMessagesViaMethod(socket, batchSize, lastSyncTimestamp) {
    ...
    for (const chat of chats.slice(0, 5)) { // Limitar a 5 chats para evitar timeout
      ...
      try {
        ...
        console.log(`[ImportManagerRobust] ⚠️ Erro ao buscar mensagens do chat: ${error.message}`);
        continue;
      }
    }
...
module.exports = ImportManagerRobust;
