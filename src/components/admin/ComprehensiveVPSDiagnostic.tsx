...
export const ComprehensiveVPSDiagnostic = () => {
  ...
  const addLog = (message: string) => {
    ...
    console.log(`[Comprehensive VPS Diagnostic] ${message}`);
  };

  const edgeFunctionTests: EdgeFunctionTest[] = [
    {
      name: "vps_diagnostic",
      description: "Diagnóstico base da VPS (conectividade)",
      endpoint: "vps_diagnostic",
      payload: { test: "vps_connectivity" }
    },
    {
      name: "vps_diagnostic_auth",
      description: "Teste de autenticação VPS via diagnóstico",
      endpoint: "vps_diagnostic", 
      payload: { test: "vps_auth" }
    },
    {
      name: "vps_diagnostic_services",
      description: "Teste de serviços VPS via diagnóstico",
      endpoint: "vps_diagnostic",
    ...
    {
      name: "whatsapp_web_server_sync",
      description: "Sincronização de instâncias",
      endpoint: "whatsapp_web_server",
      payload: { action: "sync_instances" }
    }
  ];

  const executeTest = async (test: EdgeFunctionTest): Promise<TestResult> => {
  ...
  const executeComprehensiveDiagnostic = async () => {
    ...
    try {
      ...
      console.error('[Comprehensive VPS Diagnostic] Fatal Error:', error);
    } finally {
      ...
      <Card className="bg-white/30 backdrop-blur-xl rounded-3xl border border-white/30 shadow-2xl">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Diagnóstico Completíssimo VPS - Todas Edge Functions
          </CardTitle>
          <p className="text-muted-foreground">
            Teste abrangente de todas as Edge Functions que conectam com a VPS
          </p>
        </CardHeader>
          ...
          >
            {isExecuting ? (
              ...
              <>
                ...
                Executar Diagnóstico Completíssimo
              </>
            )}
          </Button>
