-- CORREÇÃO DEFINITIVA: RPC com esquema correto e correção do texto
...
BEGIN
    ...
    -- 🎯 CORREÇÃO CRÍTICA: Texto da mensagem baseado no tipo
    CASE LOWER(COALESCE(p_media_type, 'text'))
        ...
        WHEN 'image' THEN
            v_message_text := COALESCE(NULLIF(TRIM(p_message_text), ''), '📷 Imagem');
        WHEN 'video' THEN
            v_message_text := COALESCE(NULLIF(TRIM(p_message_text), ''), '🎥 Vídeo');
        WHEN 'audio' THEN
            v_message_text := COALESCE(NULLIF(TRIM(p_message_text), ''), '🎵 Áudio');
        WHEN 'document' THEN
            v_message_text := COALESCE(NULLIF(TRIM(p_message_text), ''), '📄 Documento');
        WHEN 'sticker' THEN
            v_message_text := COALESCE(NULLIF(TRIM(p_message_text), ''), '😊 Sticker');
        ELSE
            -- ✅ FALLBACK: Usar texto se disponível, senão ícone genérico
...
EXCEPTION
    WHEN OTHERS THEN
        RETURN jsonb_build_object(
            'success', false,
            'error', SQLERRM,
            'debug_text', p_message_text,
            'debug_media_type', p_media_type
        );
END;
$function$;