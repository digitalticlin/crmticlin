-- ================================================================
-- ANÁLISE CORRIGIDA DE PADRÕES DE SALVAMENTO DE MENSAGENS V2
-- ================================================================
...
SELECT 
    ...
    (array_agg(LEFT(text, 100)))[1:5] as texto_exemplo,
    (array_agg(external_message_id))[1:5] as message_ids_exemplo
FROM messages
...
-- 7. AMOSTRA SIMPLIFICADA DE MENSAGENS POR TIPO
SELECT 
    media_type,
    COUNT(*) as total,
    COUNT(CASE WHEN media_url IS NULL THEN 1 END) as sem_url,
    COUNT(CASE WHEN media_url LIKE 'data:%' THEN 1 END) as com_base64,
    COUNT(CASE WHEN media_url LIKE '%storage%' THEN 1 END) as com_storage,
    COUNT(CASE WHEN text = '[Mensagem não suportada]' THEN 1 END) as nao_suportadas
FROM messages
...
-- 9. EXEMPLOS ESPECÍFICOS DE PROBLEMAS (TOP 10)
SELECT 
...
-- 10. RESUMO EXECUTIVO DOS PROBLEMAS
SELECT 
    'RESUMO EXECUTIVO' as titulo,
    'PROBLEMAS CRÍTICOS ENCONTRADOS' as categoria,
    COUNT(DISTINCT 
        CASE 
            WHEN media_type = 'video' AND (media_url IS NULL OR media_url NOT LIKE '%storage%') 
            THEN id 
        END
    ) as videos_sem_storage,
    COUNT(DISTINCT 
        CASE 
            WHEN text LIKE '%[Mensagem não suportada]%' 
            THEN id 
        END
    ) as mensagens_nao_suportadas,
    COUNT(DISTINCT 
        CASE 
            WHEN media_url LIKE 'data:%' 
            THEN id 
        END
    ) as midias_em_base64,
    COUNT(DISTINCT 
        CASE 
            WHEN media_type != 'text' AND media_url IS NULL 
            THEN id 
        END
    ) as midias_sem_url
FROM messages
