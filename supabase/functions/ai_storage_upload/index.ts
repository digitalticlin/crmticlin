...
console.log("🚀 AI Storage Upload Service - Direct Mode Only")
...
async function uploadMode(body: any) {
  ...
  console.log(`📤 Upload AI solicitado: ${file_path} para message_id: ${message_id}`)
  ...
  console.log(`🔄 Decodificando Base64 AI... Tamanho: ${cleanBase64.length} chars`)

  // 🔄 Decodificar Base64 para Uint8Array
  let fileBuffer: Uint8Array
  try {
    fileBuffer = Uint8Array.from(atob(cleanBase64), c => c.charCodeAt(0))
    console.log(`✅ Base64 AI decodificado com sucesso. Arquivo: ${fileBuffer.length} bytes`)
  } catch (decodeError) {
    console.error('❌ Erro ao decodificar Base64 AI:', decodeError)
    return new Response(JSON.stringify({
      success: false,
  ...
  console.log(`🚀 Fazendo upload AI para Storage: ${file_path}`)
  ...
  if (error) {
    console.error('❌ Erro no upload AI para Storage:', error)
    return new Response(JSON.stringify({
      success: false,
  ...
  console.log(`✅ Upload AI realizado com sucesso! URL: ${publicUrl}`)
  ...
  if (updateError) {
    console.error('❌ Erro ao atualizar message.media_url AI:', updateError)
    return new Response(JSON.stringify({
      success: false,
  ...
  console.log(`✅ Message AI ${message_id} atualizada com URL: ${publicUrl}`)
...
console.log("✅ AI Storage Upload Service pronto para receber requisições")