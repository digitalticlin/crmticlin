-- 🔧 CORRIGIR LÓGICA DE ATUALIZAÇÃO DE PROFILE PIC
-- Problema: profile_pic_updated permanece FALSE mesmo quando lead não tem foto
...
BEGIN
  ...
  RAISE NOTICE '[save_whatsapp_message_service_role] 🔍 Lead encontrado: % | Profile pic atual: %', 
    ...
    CASE WHEN v_current_profile_pic IS NULL THEN 'NULL' ELSE 'EXISTS' END;

  -- 📸 PROCESSAR PROFILE PIC - LÓGICA CORRIGIDA
  IF p_profile_pic_url IS NOT NULL THEN
    ...
    END IF;
  ELSE
    RAISE NOTICE '📸 Nenhum profile pic enviado no payload';
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'data', jsonb_build_object(
      ...
      'method', 'service_role_fixed_profile_pic_logic'
    )
  );
...
DO $$
BEGIN
  RAISE NOTICE '🔧 Lógica de profile pic corrigida!';
  RAISE NOTICE '✅ Novo lead: sempre processa';
  RAISE NOTICE '✅ Lead sem foto: sempre processa';
  RAISE NOTICE '✅ Foto diferente: sempre processa';
  RAISE NOTICE '✅ Foto igual: não processa';
  RAISE NOTICE '📦 Logs mais detalhados adicionados';
END $$;