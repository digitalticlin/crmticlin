...
-- CORREÇÃO CRÍTICA DO SEARCH_PATH E REFERÊNCIAS DE TABELAS
-- Forçar esquema público e adicionar diagnósticos completos
...
BEGIN
  ...
  RAISE NOTICE '[SERVICE_ROLE] 🔍 DIAGNÓSTICO - Schema: %, Path: %, Tabelas: %', 
    v_current_schema, v_search_path, v_tables_check;

  -- Log de início com diagnóstico
  RAISE NOTICE '[SERVICE_ROLE] 🚀 Iniciando SECURITY DEFINER com search_path forçado para: %', p_vps_instance_id;

  -- ETAPA 1: Buscar instância com referência absoluta
  ...
  IF v_instance_id IS NULL THEN
    ...
    RETURN jsonb_build_object(
      ...
      'method', 'service_role_security_definer_fixed',
  ...
  -- ETAPA 3: Buscar funil padrão do usuário (referência absoluta)
  ...
  -- ETAPA 4: Buscar primeiro estágio do funil (referência absoluta)
  ...
  -- ETAPA 5: Buscar ou criar lead (referência absoluta)
  ...
  IF v_lead_id IS NULL THEN
    -- Criar novo lead (referência absoluta)
  ...
  ELSE
    -- Atualizar lead existente (referência absoluta)
    UPDATE public.leads 
    SET 
  ...
  -- ETAPA 6: Inserir mensagem (referência absoluta)
...
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE '[SERVICE_ROLE] ❌ ERRO CORRIGIDO: % - SQLSTATE: %', SQLERRM, SQLSTATE;
  
  -- Se for erro de tabela não encontrada, fornecer diagnóstico detalhado
  IF SQLSTATE = '42P01' THEN
    RAISE NOTICE '[SERVICE_ROLE] 🔍 ERRO 42P01 - Tabela não encontrada. Diagnóstico: Schema=%, Path=%, Tabelas=%, Erro=%', 
      v_current_schema, v_search_path, v_tables_check, SQLERRM;
  END IF;
