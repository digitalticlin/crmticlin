...
DECLARE
  ...
  v_rate_limit INTEGER := 2; -- mensagens por minuto
BEGIN
  v_current_minute := date_trunc('minute', NOW());
  
  -- Buscar quantas mensagens foram enviadas no minuto atual
  ...
  -- Se não encontrou registro, criar um
...
BEGIN
  ...
  -- Incrementar contador do minuto atual
  INSERT INTO public.broadcast_rate_limits (
    user_id, 
    messages_sent_current_minute, 
    current_minute,
...
BEGIN
  ...
  -- Criar fila baseada no target_type
  ...
  -- Atualizar campanha com status e contadores
