-- 🚨 V4 FORCE: Forçar recriação completa da função RPC
-- Esta versão GARANTE que não há cache ou versões antigas
...
BEGIN
    ...
    -- ⚠️ LOG CRÍTICO para debug
    RAISE NOTICE '[Helper] 🔍 mime=%, type=%', v_mime, p_media_type;
...
-- 3️⃣ Recriar RPC com TODOS os 15 parâmetros
...
BEGIN
    -- ⚠️ LOG ENTRADA
    RAISE NOTICE '[RPC] 📥 ENTRADA: instance=%, phone=%, type=%, mime=%', p_vps_instance_id, p_phone, p_media_type, p_mime_type;

    -- Buscar UUID da instância
    SELECT id, user_id INTO v_instance_id, v_user_id
    FROM public.whatsapp_numbers
    WHERE vps_instance_id = p_vps_instance_id OR id = p_whatsapp_number_id
    LIMIT 1;
    ...
    IF v_lead_id IS NULL THEN
        ...
        VALUES (v_formatted_name, v_clean_phone, p_profile_pic_url, v_instance_id, v_user_id, 'webhook', v_funnel_id, v_first_stage_id, v_owner_id, NULL, p_message_text, NOW())
        RETURNING id INTO v_lead_id;
    ELSE
        UPDATE public.leads SET last_message = p_message_text, last_message_time = NOW(), profile_pic_url = COALESCE(p_profile_pic_url, profile_pic_url) WHERE id = v_lead_id;
    END IF;
    ...
    VALUES (p_message_text, p_from_me, v_user_id, v_lead_id, v_media_type_enum, NULL, p_external_message_id, v_instance_id, p_source_edge, 'webhook', NOW(), CASE WHEN p_from_me THEN 'sent'::message_status ELSE 'received'::message_status END)
    ...
    IF v_should_call_edge THEN
        ...
        -- ⚠️ LOG CRÍTICO
        RAISE NOTICE '[RPC] 🎯 EXTENSÃO CALCULADA: type=%, mime=%, ext=%', p_media_type, p_mime_type, v_file_extension;
        ...
        BEGIN
            -- ⚠️ LOG DO FILE_PATH EXATO
            RAISE NOTICE '[RPC] 📤 Chamando edge com file_path: webhook/%/%', v_instance_id, v_message_id || '.' || v_file_extension;
            ...
            RAISE NOTICE '[RPC] ✅ Edge respondeu: %', v_edge_result;
...
DO $$
BEGIN
    RAISE NOTICE '✅ V4 FORCE aplicada com LOGS DETALHADOS!';
    RAISE NOTICE '🔍 Agora todos os logs do RPC aparecerão no Dashboard';
END $$;
