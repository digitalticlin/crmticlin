-- Corrigir RPC save_sent_message_from_app para salvar corretamente
-- Problema: created_by_user_id recebendo UUID da instância ao invés do usuário
...
CREATE OR REPLACE FUNCTION public.save_sent_message_from_app(
    ...
    p_source_edge text DEFAULT 'whatsapp_messaging_service'::text
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    IF p_media_type != 'text' AND p_base64_data IS NOT NULL AND LENGTH(p_base64_data) > 100 THEN
        ...
        v_file_path := format('app/%s/%s/msg_%s_%s.%s',
            p_media_type,
        ...
        BEGIN
            SELECT net.http_post(
                'https://rhjgagzstjzynvrakdyj.supabase.co/functions/v1/app_storage_upload',
                jsonb_build_object(
            ...
            RAISE NOTICE '📤 Upload APP iniciado para message_id: % (request_id: %)', v_message_id, v_request_id;

        EXCEPTION
            WHEN OTHERS THEN
                RAISE NOTICE '⚠️ Erro ao iniciar upload APP: %', SQLERRM;
        END;
    END IF;
    ...
    RETURN jsonb_build_object(
        ...
        'processing_mode', 'optimized_async_app',
