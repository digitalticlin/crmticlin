...
console.log('[Webhook] 🚀 Inicializando webhook WhatsApp Web v3.1 - DEBUG MODE');
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (sanitizedData.messageType !== 'text') {
      ...
      // 🚨 BUSCA RECURSIVA PARA ENCONTRAR DADOS BASE64
      console.log('[Webhook] 🔍 BUSCA RECURSIVA DE BASE64:', findBase64InObject(sanitizedData, ''));
    }
...
// 🚨 FUNÇÃO PARA BUSCAR BASE64 RECURSIVAMENTE
function findBase64InObject(obj: any, path: string, maxDepth = 5): any[] {
  const results: any[] = [];
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    if (typeof value === 'string') {
      // Verificar se parece com base64 (mais de 100 chars e caracteres válidos)
      ...
      results.push(...findBase64InObject(value, currentPath, maxDepth - 1));
    }
  }
...
async function processMessage(supabase: any, data: any) {
  ...
  if (messageData.mediaData?.base64Data && messageData.messageType !== 'text') {
    ...
    console.log('[Webhook] ⚠️ Mídia não processada:', {
      hasBase64: !!messageData.mediaData?.base64Data,
      messageType: messageData.messageType,
      base64Length: messageData.mediaData?.base64Data?.length || 0
    });
  }
