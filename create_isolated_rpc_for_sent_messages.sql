-- ========================================
-- RPC ISOLADA PARA WHATSAPP_MESSAGING_SERVICE
-- Função específica para salvar APENAS mensagens enviadas (from_me = true)
-- ========================================
...
BEGIN
  ...
  RAISE NOTICE '[SENT_MESSAGE_ONLY] 🚀 Salvando mensagem ENVIADA para: %', p_vps_instance_id;
  ...
  -- ETAPA 6: Inserir mensagem ENVIADA (from_me = TRUE)
  ...
  -- ETAPA 7: Retornar resultado específico
  RETURN jsonb_build_object(
    'success', true,
...
GRANT EXECUTE ON FUNCTION public.save_sent_message_only(text, text, text, text, text) TO service_role;
GRANT EXECUTE ON FUNCTION public.save_sent_message_only(text, text, text, text, text) TO authenticated; 