-- SOLUÇÃO FINAL: Remover RPC antiga e criar nova com correções completas

-- 1. REMOVER TODAS AS VERSÕES DA FUNÇÃO
DROP FUNCTION IF EXISTS public.save_received_message_webhook(uuid, text, text, boolean, text, text, text, text, text, text, text, text, uuid, text);
DROP FUNCTION IF EXISTS public.save_received_message_webhook(text, text, text, boolean, text, text, text, text, text, text, text, text, uuid, text);

-- 2. CRIAR FUNÇÃO CORRIGIDA COM NOME DA INSTÂNCIA
CREATE OR REPLACE FUNCTION public.save_received_message_webhook(
    p_vps_instance_id text,         -- ✅ MANTÉM NOME ORIGINAL mas tipo text
    p_phone text,
    p_message_text text,
    p_from_me boolean,
    p_media_type text,
...
BEGIN
    ...
    -- 🎯 CORREÇÃO CRÍTICA: Definir texto da mensagem CORRETAMENTE
    CASE LOWER(COALESCE(p_media_type, 'text'))
        WHEN 'text' THEN
            -- ✅ MENSAGEM DE TEXTO - SEMPRE USAR CONTEÚDO REAL
        ...
        ELSE
            -- ✅ FALLBACK: Se tipo desconhecido mas tem texto, usar texto
            v_message_text := CASE
                WHEN COALESCE(NULLIF(TRIM(p_message_text), ''), '') != '' THEN p_message_text
                ELSE '📎 Mídia'
            END;
    END CASE;
    ...
    IF v_lead_id IS NULL THEN
        ...
        INSERT INTO public.leads (
            ...
            v_owner_id,
            NULL,
            v_message_text,  -- ✅ ÚLTIMA MENSAGEM CORRETA
            NOW()
        )
    ...
    INSERT INTO public.messages (
        ...
        v_message_text,  -- ✅ TEXTO CORRETO DA MENSAGEM
        p_from_me,
        v_user_id,
        v_lead_id,
        v_media_type_enum,
        ...
        v_instance_id,   -- ✅ UUID DA INSTÂNCIA
        p_source_edge
    )
    ...
    RETURN jsonb_build_object(
        ...
        'message_text', v_message_text,  -- ✅ DEBUG: mostrar texto salvo
