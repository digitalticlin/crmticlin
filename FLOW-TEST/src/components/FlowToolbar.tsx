...
export const FlowToolbar = ({ onAddNode, designStyle }: FlowToolbarProps) => {
  ...
  const nodes = [
    { type: 'start' as const, icon: Play, label: 'Start', color: 'text-emerald-500' },
    { type: 'action' as const, icon: Zap, label: 'Action', color: 'text-purple-500' },
    { type: 'decision' as const, icon: GitBranch, label: 'Decision', color: 'text-amber-500' },
    { type: 'end' as const, icon: StopCircle, label: 'End', color: 'text-red-500' },
  ];
