-- Criar RPC save_sent_message_from_ai para a Edge ai_messaging_service
-- Segue o mesmo padrão das RPCs de app e webhook

CREATE OR REPLACE FUNCTION public.save_sent_message_from_ai(
    p_vps_instance_id text,  -- AI usa texto para instance ID
    p_phone text,
    p_message_text text,
    p_from_me boolean,
    p_media_type text,
...
BEGIN
    ...
    -- 🔍 BUSCAR INSTÂNCIA PELO NOME (AI usa instance_name como ID)
    SELECT id, created_by_user_id
    INTO v_instance_id, v_user_id
    FROM public.whatsapp_instances
    WHERE instance_name = p_vps_instance_id
       OR vps_instance_id::text = p_vps_instance_id
    LIMIT 1;
    ...
    IF v_instance_id IS NULL THEN
        RETURN jsonb_build_object(
            ...
            'instance_id_searched', p_vps_instance_id
        );
    END IF;
    ...
    IF p_media_type != 'text' AND p_base64_data IS NOT NULL AND LENGTH(p_base64_data) > 100 THEN
        ...
        -- 📤 CHAMAR EDGE ai_storage_upload (sem aguardar resposta - assíncrono)
        BEGIN
